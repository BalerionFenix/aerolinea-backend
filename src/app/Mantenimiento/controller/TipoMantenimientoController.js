...
import {
    TipoMantenimientoInputDTO,
    TipoMantenimientoUpdateDTO,
    TipoMantenimientoOutputDTO
...
export const actualizarTipoMantenimiento = async (req, res) => {
    try {
        ...
        if (updateDTO.nombre) {
            const nombreExiste = await TipoMantenimientoDAO.existeNombre(
                updateDTO.nombre,
                id
            );
            if (nombreExiste) {
